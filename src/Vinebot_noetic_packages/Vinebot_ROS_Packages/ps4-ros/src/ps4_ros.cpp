...
public:
    ...
    PS4_ROS(ros::NodeHandle &n) {
        ...
        private_nh.param<std::string>("pub_topic", this->pubName, "/vinebot_controller/cmd_vel");
        private_nh.param<std::string>("emergency_topic", this->emergencyName, "/emergency");

        this->chat = n.advertise<geometry_msgs::Twist>(pubName, 1000);
        this->emergency_stop = n.advertise<std_msgs::Bool>(emergencyName, 1000);
    ...
    void publishTwistMsg() {
        ...
        //printRaw();
        if(!this->buttonTouch) {
             if ((this->buttonX==true) && (this->send_l2 >= 0.01) && (this->send_l2 <= maxVel)){
               msg.linear.x = this->send_l2;

            } else if ((this->buttonX==true) && (this->send_r2 <= 0.0) && (this->send_r2 >= maxVelR)) {
                msg.linear.x = this->send_r2;

            }   else if ((this->buttonSq==true) && (this->send_l2 >= 0.01) && (this->send_l2 <= maxVel)){
                msg.linear.x = this->send_l2/5;
        
            }  else if ((this->buttonSq==true) && (this->send_r2 <= 0.0) && (this->send_r2 >= maxVelR)){
                msg.linear.x = this->send_r2/5;

            } else if ((this->send_l2 >= 0.01) && (this->send_l2 <= maxVel)) {
                msg.linear.x = this->send_l2/10;

            } else if ((this->send_r2 <= 0.0) && (this->send_r2 >= maxVelR)) {
                msg.linear.x = this->send_r2/10;

            } 
            
            // Drehbewegung
            msg.angular.z = this->send_leftStickX / 10;
    }
    ...
    void emergency()
    {
        ...
        this->emergency_stop.publish(emergency_msg);
...
private:
    ...
    int arrowsX, arrowsY, buttonSq, buttonO, buttonTr,buttonTouch, l1, r1;
    //int buttonX;
    bool buttonX = false;
